...
!     ------------------------------------------------------------------
    integer(ip)            :: i, j, k, klast, l, l1, l2, lenl, nrank
...
!     ------------------------------------------------------------------
    integer(ip)            :: nout, lprint, imax, jmax, lamin, nrank, &
                              lenl, lenu, lrow, ncp, mersum, mode, &
                              i, j, k
    real(rp)               :: amax, elmax, umax, dumin
    ...
    jmax   = n
    if (lprint <= 0) return
    if (lprint <= 2) imax = min( imax, 10 )
    if (lprint <= 2) jmax = min( jmax, 10 )
...
!     --------------------------------
    ...
    write(nout, 2100) (p(i), i = 1, imax)
    write(nout, 2200) (q(j), j = 1, jmax)
    if (lprint <= 1) return
...
!     -------------------------------------------------------
    ...
    '    growth', e9.1)
    2100 format(//' Row    permutation  p' / (1x, 10i7))
    2200 format(//' Column permutation  q' / (1x, 10i7))
...
!     with identities for the row and column permutations p, q.
...
!     ------------------------------------------------------------------
    ...
    integer(ip)          :: i, j, jsing, l, lenl, lenu, lrow, minmn, ncp, &
                            nrank, nsing, numl0, nupdat
    real(rp),  parameter :: zero = 0.0
...
! The permutations p and q point to the nonsingular part of w.
! Fix up the remaining parts of p.

    do 200 i = 1, m
        if (  i  >  n  ) go to 150
        if (w(i) /= zero) go to 200
        150 l       = l + 1
        p(l)   = i
        lenr(i) = 0
        nsing   = nsing + 1
